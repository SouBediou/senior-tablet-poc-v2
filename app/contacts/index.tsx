...
export default function ContactsScreen() {
  ...
  return (
    <View style={{ flex: 1, padding: 16, gap: 12 }}>
      {/* Titre encadré jaune (style "badge") */}
  ...
  <UiText muted style={{ textAlign: "center" }}>
    Ajouter un proche ou un soignant
        ...
        {(["all", "family", "care", "other"] as const).map((k) => {
          ...
          return (
            <Pressable
              ...
              accessibilityLabel={
                k === "all" ? "Filtrer tous les contacts" : `Filtrer ${ROLE_LABEL[k]}`
              }
              style={{
                paddingVertical: 10,
      ...
      {/* Ajouter contact (plus visible) */}
      <Pressable
        onPress={() => router.push("/contacts/edit")}
        accessibilityRole="button"
        accessibilityLabel="Ajouter un contact"
        accessibilityHint="Ouvre le formulaire d’ajout"
        style={{
          padding: 16,
          borderRadius: 18,
          borderWidth: 2,
        ...
        {loading ? (
          ...
          filtered.map((c) => (
            <Pressable
              key={c.id}
              onPress={() =>
                router.push({ pathname: "/contacts/edit", params: { id: c.id } })
              }
              accessibilityRole="button"
              accessibilityLabel={`Modifier ${c.name}`}
              accessibilityHint="Ouvre l’écran de modification"
              style={{
                padding: 16,
                borderRadius: 18,
                borderWidth: 2,
